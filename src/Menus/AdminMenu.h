...
#include "../Data/Handlers/FileHandler.h"
...
void EditEmployeeInformation()
{
    ...
    char newName[NAME_MAX_LENGTH], oldPassword[PASSWORD_MAX_LENGTH];
    float newBalance = 0;
    ...
    while (TRUE) {
        ...
        if (CheckEmployee(&employee, employeeID)) {
            ...
            while (TRUE) {
                printf("\t\t\tWhat you want to change (0-Exit, 1-Name 2-Surname, 3-Role, 4-Password): ");
            ...
            switch (userChoice)
            {
                ...
                case 3:
                    while (TRUE) {
                        printf("%sEnter role(1-admin, 2-employee):", PADDINGLEFT);
                        scanf(" %d", &employee->Data.Role);

                        if (employee->Data.Role > 0 && employee->Data.Role < 3) {
                            break;
                        }
                        printf("\033[A\33[2K");
                    }
...
void SearchEmployee()
{
    ...
    while (TRUE)
    {
        ...
        if (CheckEmployee(&employee, employeeID)) {
            ...
            printf("%sEMPLOYEE ID         NAME                SURNAME             ROLE\n\t\t        _______________________________________________________________________\n\n", LISTOFFSET);
            printf("\n\t\t\t         %d", employee->Data.ID);
            PrintSpaces(LISTROWLENGTH - 3 - CountDigits(employee->Data.ID));
            ...
            printf("%s", (employee->Data.Role == ADMIN) ? "ADMIN" : "EMPLOYEE");
